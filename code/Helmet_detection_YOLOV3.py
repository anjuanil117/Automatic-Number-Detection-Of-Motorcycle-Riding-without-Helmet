...
# Remove the bounding boxes with low confidence using non-maxima suppression
    ...
    # lower confidences.
        ...
        if count_person == 0:
            path = 'Images/WithoutHelmet/'
            frame_name=os.path.basename(fn)# trimm the path and give file name.
            frame_name1=(str(a)+frame_name)
            cv.imwrite(str(path)+frame_name1, frame)     # writing to folder.
            #cv.imshow('img',frame)
            Main.main(frame,frame_name1)
            cv.waitKey(800)
        else:
            path = 'Images/WithHelmet/'
            frame_name=os.path.basename(fn) # trimm the path and give file name.
            frame_name1=(str(a)+frame_name)
            cv.imwrite(str(path)+frame_name1, frame)     # writing to folder.
            #print(type(frame))
